/* ==========================================================================
   ...
   Gerçek SMS + Manuel Onaylı PDF + Şantiye Hafızası
   ========================================================================== */
...
// ==========================================================================
// 1. KÖPRÜ MOTORLARI (Arayüz Butonlarını auth.js'ye Bağlar)
// ==========================================================================
export const AUTH = {
    loginWithGoogle: async () => {
        const userData = await googleIleGiris();
        if (userData) {
            window.location.reload(); // Giriş başarılıysa sayfayı yenile ve stadyumu aç
        }
    },
    ...
    verifyPdf: async () => {
        ...
        try {
            ...
            UI.showToast('Belgeniz başarıyla okundu! Yönetim kurulundan onay bekleniyor.', 'success');
            UI.closeModal('pdf-modal');
            setTimeout(() => window.location.reload(), 1500); // UI'ı 'Onay Bekliyor' moduna almak için yenile
        } catch (error) {
    ...
    verifyPhone: async () => {
        ...
        btn.innerHTML = 'SMS GÖNDERİLİYOR...'; 
        btn.disabled = true;

        try {
            await gercekSmsGonder(phoneValue, 'btn-submit-phone');
            UI.showToast('SMS başarıyla gönderildi! Lütfen telefonunuza gelen kodu girin.', 'success');
        } catch (error) {
            UI.showToast('SMS Gönderilemedi. Lütfen numaranın başına 0 koymadan veya +90 ekleyerek deneyin.', 'error');
        } finally {
    ...
    verifyOtp: async () => {
        ...
        if(!otpValue || otpValue.length < 6) { 
            UI.showToast('Lütfen 6 haneli doğrulama kodunu eksiksiz girin.', 'error'); 
            return; 
        }
        ...
        try {
            ...
            UI.showToast('Telefon numaranız başarıyla onaylandı ve kaydedildi!', 'success');
            STATE.updateUser('hasPhone', true);
            UI.closeModal('phone-modal');
            Me26VotingSystem.updateVisibility();
        } catch (error) {
            UI.showToast('Hatalı veya süresi geçmiş kod girdiniz!', 'error');
        } finally {
...
export const Me26VotingSystem = {
    ...
    updateVisibility: function() {
        ...
        if (STATE.isLoggedIn()) {
            ...
            // 4. PDF (E-DEVLET) BUTONUNU YÖNET (Manuel Onay Algoritması)
            if (pdfBtn) {
                if (!hasPhone) {
    ...
    handleVote: async function(btnEl) {
        ...
        if (currentPower === 0) {
            UI.showToast('Geçersiz Oy Gücü! Profil panelinden e-devlet belgenizi yükleyip onay almalısınız.', 'error');
            return;
        }
...
// ==========================================================================
// 3. SAYFA YÜKLENDİĞİNDE ÇALIŞACAK MOTORLAR (DOM Dinleyicileri)
// ==========================================================================
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const copyLinkToClipboard = async () => {
        ...
        try {
            ...
            UI.showToast('Link kopyalanamadı, manuel seçerek kopyalayın.', 'error');
        }
    };
    ...
    // ==========================================================================
    // 4. ŞANTİYE HAFIZASI: Panik yapmaz (.maybeSingle), Yönetim Kararlarını Uygular
    // ==========================================================================
